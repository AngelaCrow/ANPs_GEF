...
df0.05_0.7<-filter(Pvalue.05, tau.Promedio >= 0.8)

###tau_area_log####
ggplot(tb_vars, aes(x = tau.Área_log, y = tau.Promedio,label =z.ID_anp )) +
  geom_point(size = 1) +  geom_point(color="#006d2c")+
  geom_text_repel(aes(label=NOMBRE), size = 3)+
  facet_grid(MGC~.) +
  geom_errorbar(aes(ymin = tau.variable_min, ymax = tau.variable_max), color="#006d2c")
  #geom_errorbarh(aes(xmax = Pvalue.variable_max, xmin = Pvalue.variable_min))
ggsave("tmin_graficas/Tau_rcp85.jpg", dpi = 300)

ggplot(Pvalue.05, aes(x = tau.Área_log, y = tau.Promedio,label =z.ID_anp )) +
  geom_point(size = 1) +  geom_point(color="#41ae76")+
  geom_text_repel(aes(label=NOMBRE), size = 3)+
  facet_grid(MGC~.) +
  geom_errorbar(aes(ymin = tau.variable_min, ymax = tau.variable_max), color="#41ae76")
#geom_errorbarh(aes(xmax = Pvalue.variable_max, xmin = Pvalue.variable_min))
ggsave("tmin_graficas/Pvalue.05_RCP85.jpg", dpi = 300)

ggplot(df0.05_0.7, aes(x = tau.area_log, y = tau.Promedio,label =z.ID_anp )) +
  geom_point(size = 1) +  geom_point(color="#2ca25f")+
...
###tau_pvalue####
ggplot(tb_vars, aes(x = Pvalue.Promedio, y = tau.Promedio,label =z.ID_anp )) +
  geom_point(size = 1) +  geom_point(color="#2ca25f")+
  #geom_text_repel(aes(label=z.ID_anp), size = 3)+
  facet_grid(MGC~.) +
...
ggplot(df0.05_0.7, aes(x = Pvalue.Promedio, y = tau.Promedio,label =z.ID_anp )) +
  geom_point(size = 1) +  geom_point(color="#2ca25f")+
  #geom_text_repel(aes(label=z.ID_anp), size = 3)+
  facet_grid(MGC~.)+ 
  geom_errorbar(aes(ymin = tau.variable_min, ymax = tau.variable_max))+
